...
function actualizeazaBadgeCos() {
    ...
    const badge = document.querySelector('.nav-icon .badge'); // Selectează badge-ul

    if (badge) {
        badge.textContent = cos.length; // Actualizează numărul de produse din coș
        badge.classList.add('badge-update'); // Adaugă o clasă pentru animație
        setTimeout(() => badge.classList.remove('badge-update'), 500); // Elimină animația după 500ms
    }
}
...
menuToggle.addEventListener('click', function() {
    ...
    menuToggle.classList.toggle('rotate-icon'); // Adaugă efect de rotație
});
